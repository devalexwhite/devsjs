...
export default function Page2048() {
  ...
  return (
    ...
    >
      ...
      >
        <strong>Instructions:</strong> Use WASD to move tiles Up, Down, Left,
        Right. Combine same numbers to increase your score!
      </p>
    </div>
  );
}
...
const update = () => {
  ...
  if (
    Math.abs(animOffset.x) > SQUARE_SIZE + GAP ||
    Math.abs(animOffset.y) > SQUARE_SIZE + GAP
  ) {
    ...
    for (let x = 0; x < grid.length; x++) {
      for (let y = 0; y < grid.length; y++) {
        if (canMove(x, y) && grid[x][y] > 0) {
          didMove = true;
          if (
            grid[x + moveX][y + moveY] === grid[x][y] &&
            !canMove(x + moveX, y + moveY)
          ) {
            score += grid[x][y] * 2;
...
const drawPieces = (context) => {
  for (let x = 0; x < grid.length; x++) {
    for (let y = 0; y < grid.length; y++) {
      ...
      context.fillRect(
        x * (SQUARE_SIZE + GAP) + (cellCanMove ? animOffset.x : 0),
        y * (SQUARE_SIZE + GAP) + (cellCanMove ? animOffset.y : 0),
        SQUARE_SIZE,
        SQUARE_SIZE
      );
      ...
      context.fillText(
        ...
        x * (SQUARE_SIZE + GAP) +
          SQUARE_SIZE / 2 +
          (cellCanMove ? animOffset.x : 0),
        y * (SQUARE_SIZE + GAP) +
          SQUARE_SIZE / 2 +
          (cellCanMove ? animOffset.y : 0)
      );
    }
  }
};
